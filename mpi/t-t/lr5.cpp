...
void measureTimePar(vector<vector<double>>& matrix, int numMeasurements) {
    ...
    int localN = matrix.size();
    int m = (localN > 0 ? matrix[0].size() : 0);
    ...
    for (int r = 0; r < numMeasurements; ++r) {
        ...
        if (rank == 0) {
            ...
            for (int src = 1; src < size; ++src) {
                MPI_Recv(&recvVal, 1, MPI_DOUBLE,
                         src, 0, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE);
                globalSum += recvVal;
            }
        } else {
            MPI_Send(&localSum, 1, MPI_DOUBLE,
                     0, 0, MPI_COMM_WORLD);
        }

        double end = MPI_Wtime();
        totalTime += (end - start) * 1e6; // мкс
    }

    double avgTime = totalTime / numMeasurements;

    if (rank == 0)
        cout << " | Параллельный MPI (точка-точка): " << avgTime << " мкс; " << "max=" << globalSum << endl;
}
